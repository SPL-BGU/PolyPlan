...
from utils.wooden_sword.problem_generator import ProblemGenerator
import json
...
def generate_solutions_basic(output_directory_path: Path, problem_index: int) -> bool:
    domain = str(Path("planning/wooden_sword/basic_minecraft_domain.pddl").absolute())
...
def generate_solutions_advanced(
    ...
    domain = str(
        Path("planning/wooden_sword/advanced_minecraft_domain.pddl").absolute()
    )
...
def valid_problem_basic(output_directory_path: Path, problem_index: int) -> bool:
    env = BasicMinecraft(
        decoder_class=MacroActionsDecoder, start_pal=False, keep_alive=True
    )
